...
library(ggsci)
library(gridExtra)
# switch A and B, B more interesting
setwd('C:/Users/magnus/Desktop/Next Generation Proteomics')

## read data and construct data frames for panel A
A10<-as.matrix(read.csv("seed1_max10_missed1.csv", header = FALSE, sep = ";"), byrow = TRUE)
A20<-as.matrix(read.csv("seed1_max20_missed1.csv", header = FALSE, sep = ";"), byrow = TRUE)
A50<-as.matrix(read.csv("seed1_max50_missed1.csv", header = FALSE, sep = ";"), byrow = TRUE)
A<-rbind(A10, A20, A50)
...
B0<-as.matrix(read.csv("seed1_max50_missed0.csv", header = FALSE, sep = ";"), byrow = TRUE)
B1<-as.matrix(read.csv("seed1_max50_missed1.csv", header = FALSE, sep = ";"), byrow = TRUE)
B<-rbind(B0, B1)
...
C <- rbind(C4, C10)
dfC1 <- data.frame(x = rep(5*(1:20), 21),
                   y = as.vector(t(C)),
                   n_read = c(rep("4", 400), rep("10", 20)))
...
for (i in 1:dim(D)[1]) {
  ...
  nV[i] <- sum(charToRaw(D[i,1]) == charToRaw('V')) # Why still NA? Try rt?
  nX[i] <- 1
}
